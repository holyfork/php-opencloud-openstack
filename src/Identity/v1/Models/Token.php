...
class Token extends OperatorResource implements Creatable, Retrievable, \OpenStack\Common\Auth\Token
{
    ...
    {
        $this->id = $response->getHeaderLine('X-Auth-Token');
        $this->expires = new \DateTimeImmutable('+' . $response->getHeaderLine('X-Auth-Token-Expires') . ' seconds');
        $this->catalog = new Catalog($this->client, $this->api);
        $service = new Service($this->client, $this->api);
        $service->name = 'swift';
        $service->type = 'object-store';
        $endpoint = new Endpoint($this->client, $this->api);
        $endpoint->url = $response->getHeaderLine('X-Storage-Url');
        $endpoint->interface = 'public';
        $endpoint->region = '';
        $endpoint->name = 'swift';
        $service->endpoints[] = $endpoint;
        $this->catalog->services[] = $service;
    ...
    /**
     * @param array $userOptions {@see \OpenStack\Identity\v1\Api::postTokens}
     */
